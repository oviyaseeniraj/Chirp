...
class RangeDoppler:
    ...
    def process(self):
        ...
        # Save cleaned, Doppler-shifted complex RDM for angle estimation.
        self.clean_rdm_complex = rdm.copy()

        # Linear power: abs(RD)^2
        mag2 = rdm.real * rdm.real + rdm.imag * rdm.imag

        # Average power across virtual channels
        power_avg = mag2.reshape(
            config.RX * config.TX,
            config.SLOW_TIME * config.FAST_TIME,
        ).mean(axis=0)

        self.power_map = power_avg.reshape(
            (config.SLOW_TIME, config.FAST_TIME)
        ).astype(np.float32)

        # MATLAB-style dB power
        self.power_db_map = (
            10.0 * np.log10(self.power_map + 1e-10)
        ).astype(np.float32)

        # Visualization-only normalized dB image
        display = self.power_db_map.copy()
        mn = display.min()
        mx = display.max()

        if mx > mn:
            display = (display - mn) * (255.0 / (mx - mn))
        else:
            display[:] = 0.0

        self.display_map = display.astype(np.float32)
    ...
    def get_clean_rdm(self):
        ...
        rdm_complex = self.clean_rdm_complex.copy()  # Shape: (TX * RX, SLOW_TIME, FAST_TIME)

        # Reshape from (TX * RX, SLOW_TIME, FAST_TIME)
        # to (TX, RX, SLOW_TIME, FAST_TIME)
        rdm_reshaped = rdm_complex.reshape(
            (config.TX, config.RX, config.SLOW_TIME, config.FAST_TIME)
        )

        # Transpose to (SLOW_TIME, RX, TX, FAST_TIME)
        # to match angle.py expectations.
