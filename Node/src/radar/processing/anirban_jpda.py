...
class StoneSoupJPDATracker:
    ...
    def compute_soft_associations(self, detections: List[Detection]): 
        
        prob_track_was_detected = []
        prob_detection_is_clutter = []

        prob_track_was_missed = []
        prob_track_was_hit = []

        #track, detection
        distances = [ [self._maha_dist( State(self._measurement_to_state(d)), track.states[-1]) for d in detections] for track_id, track in self.tracks.items()]
    ...
    def process_frame(self,
        ...
        if self.tracks:
            ...
            associations = self.compute_soft_associations(detections)
