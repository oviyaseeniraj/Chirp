...
class FourNodeRadarCalibration:
    ...
    def plot_ground_truth(self, complex_trajectory_array):
        """Plot ground truth scenario with refined visuals and individual color-coded FOVs"""
        ...
        # Colors for radars and FOVs
        c_vals = np.linspace(0, 1, self.num_radars)
        radar_colors = [plt.cm.viridis(val) for val in c_vals]
        
        for index in range(self.num_radars):
            radar_pos = (self.complex_radar_position_array[index].real, 
                         self.complex_radar_position_array[index].imag)
            radar_orient = self.radar_orientation_array[index]
            
            # Create color-coded FOV wedge
            wedge = Wedge(radar_pos, fov_range, 
                          radar_orient - fov_angle/2, 
                          radar_orient + fov_angle/2,
                          color=radar_colors[index], alpha=0.15, zorder=0)
            plt.gca().add_patch(wedge)
            
            # Plot FOV boundary lines
            angle_rad_1 = np.deg2rad(radar_orient - fov_angle/2)
            angle_rad_2 = np.deg2rad(radar_orient + fov_angle/2)
            plt.plot([radar_pos[0], radar_pos[0] + fov_range * np.cos(angle_rad_1)],
                     [radar_pos[1], radar_pos[1] + fov_range * np.sin(angle_rad_1)],
                     color=radar_colors[index], alpha=0.3, linewidth=1.5, zorder=0)
            plt.plot([radar_pos[0], radar_pos[0] + fov_range * np.cos(angle_rad_2)],
                     [radar_pos[1], radar_pos[1] + fov_range * np.sin(angle_rad_2)],
                     color=radar_colors[index], alpha=0.3, linewidth=1.5, zorder=0)

        # Plot trajectory
        plt.plot(complex_trajectory_array.real, complex_trajectory_array.imag, 
                'blue', linewidth=3, label='Target Trajectory', zorder=2)
        # Mark start of trajectory
        plt.scatter(complex_trajectory_array[0].real, complex_trajectory_array[0].imag,
                   color='darkblue', s=150, marker='o', label='Start Position', zorder=4)
        
        # Plot radar positions
        for index in range(self.num_radars):
            label = 'Radar Positions' if index == 0 else ""
            plt.scatter(self.complex_radar_position_array[index].real, 
                       self.complex_radar_position_array[index].imag, 
                       color=radar_colors[index], s=300, label=label, 
            ...
            plt.text(self.complex_radar_position_array[index].real + 2, 
                    ...
                    text_string, fontsize=26, fontweight='bold', 
                    bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=2))
        
        # Plot orientations - Skinny arrows
        U = 15 * np.cos(np.deg2rad(self.radar_orientation_array))
        V = 15 * np.sin(np.deg2rad(self.radar_orientation_array))
        plt.quiver(self.complex_radar_position_array.real, 
                  ...
                  color='red', width=0.005, label='Radar Orientations', zorder=6)
        
        # Draw Overlapping FOV perimeter
        ...
        plt.plot(plot_radar_position_array[0, :], plot_radar_position_array[1, :], 
                'k--', linewidth=2, alpha=0.6, label='Detection Area Perimeter')
        
        plt.legend(loc='upper left', fontsize=20, framealpha=0.9, edgecolor='black')
        plt.xlabel("X-Axis (m)", fontsize=26, fontweight='bold')
        plt.ylabel("Y-Axis (m)", fontsize=26, fontweight='bold')
        plt.title(f"Ground Truth: 4-Radar {self.config.upper()} Configuration", 
        ...
        all_x = np.concatenate([self.radar_position_array[0, :], complex_trajectory_array.real])
        all_y = np.concatenate([self.radar_position_array[1, :], complex_trajectory_array.imag])
        plt.xlim(np.min(all_x) - 15, np.max(all_x) + 15)
        plt.ylim(np.min(all_y) - 15, np.max(all_y) + 15)
