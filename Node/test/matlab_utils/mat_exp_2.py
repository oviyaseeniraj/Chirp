...
def main():
    ...
    for i in range(num_frames):
        # Get frame data
        raw_file = os.path.join(raw_dir, f"raw_frame_{i+1:04d}.npy")
        if not os.path.exists(raw_file):
            print(f"Raw file {raw_file} not found. Stopping.")
            break
            
        frame_data = np.load(raw_file)
        ...
        cube = cube.reshape((TX, RX, SLOW_TIME, FAST_TIME))
        #
        ...
        all_frames.append(transposed_cube.copy())

    final_data = np.stack(all_frames, axis=0)

    # Save to .mat file
    output_file = "plz_work.mat"
