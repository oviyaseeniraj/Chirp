...
def main():
    parser = argparse.ArgumentParser(
        description="Run playback test using modular pipeline."
    )
    parser.add_argument(
        "--input-dir", required=True, help="Directory containing .npy raw frames"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--clusters-only",
        action="store_true",
        help="Visualize only clustered centroids (clean view)",
    )
    ...
    print(
        f"Playback started from {args.input_dir}. Processes: {p_daq.pid}, {p_proc.pid}, {p_post_dbscan.pid}, {p_sock.pid}"
    )
