...
class NodeTriggerClient:
    def __init__(self) -> None:
        ...
        self.worker_cfg = WorkerConfig(
            ...
            max_pulses=_env_int("TRIGGER_MAX_PULSES", -1) if os.getenv("TRIGGER_MAX_PULSES") else None,
            cpu_core=_env_int("TRIGGER_CPU_CORE", 5) if os.getenv("TRIGGER_CPU_CORE") else None,
        )
    ...
    def _publish_ack(
        ...
        *,
        ...
        latency_ms: Optional[int], # computes the time between laptop start request & time to produce an ACK
        ...
        self.client.publish(self.ack_topic, payload=json.dumps(payload), qos=1, retain=False)
    ...
    def _handle_start(self, payload: Dict[str, Any]) -> None:
        ...
        with self.worker_lock:
            ...
            argv: List[str] = [self.worker_cfg.binary_path, "--start-epoch-ms", str(start_epoch_ms_int)]
            ...
            try:
                ...
                self._publish_state("error", command_id=command_id, error=f"worker_launch_failed:{exc}")
    ...
    def _watch_worker(
        ...
        if exit_code == 0:
            ...
            logging.warning("Worker exited with error code=%d commandId=%s", exit_code, command_id)

    def _presence_loop(self) -> None:
        while not self.shutdown_event.is_set():
            self._publish_presence(status="online")
            self.shutdown_event.wait(self.presence_heartbeat_ms / 1000.0) # sleep presence "heartbeat" for 2 seconds

    def run(self) -> None:
        self.client.connect(self.mqtt_host, self.mqtt_port, keepalive=self.keepalive_sec)
